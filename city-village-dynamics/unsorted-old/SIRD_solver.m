function dydt = SIRD_solver(t,y,n_age_cat,n_eco_cat,imports)
...
%     end
%     
    contact_map = R0 * ones(n_age_cat,n_eco_cat,n_age_cat,n_eco_cat) / ti;
...
% import_R(1:2,2) = 0.1*import_rate*[0.50; 0.30; 0.20];
    ...
    for j = 1 : n_eco_cat
        for i = 1 : n_age_cat
            ...
            dSdt(i,j) = - 1/N * S(i,j) * sum(sum(beta.*I)) - p(i,j) * S(i,j)/tp + import_S(i,j);
            dIdt(i,j) = 1/N * S(i,j) * sum(sum(beta.*I)) - I(i,j)/ti + import_I(i,j);
    ...
    for j = 1 : n_eco_cat
        for i = 1 : n_age_cat
            tmp = [dSdt(i,j) dIdt(i,j) dRdt(i,j) dDdt(i,j)];
            dydt = [dydt tmp];
        end
    end
%     dydt = dydt.';
